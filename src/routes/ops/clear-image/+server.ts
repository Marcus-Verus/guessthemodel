...
import { clearItemImage } from '$lib/server/daily';
import { dayForNumber } from '$lib/duped';

/** Remove a card's image. Body: { n, index }. Gated by ADMIN_SECRET. */
export const POST: RequestHandler = async ({ url, cookies, request }) => {
	...
	const result = await clearItemImage(dayForNumber(n), index);
	return json(result);
};
